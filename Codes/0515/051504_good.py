...
def controlMotor(target_distance, dt):
    ...
    if distance is not None:
        ...
        sim.setJointTargetPosition(motor_handle, control_signal/100)
        last_error = error
...
def run_simulation():
    ...
    while time.time() - start_time < simulation_time:
        ...
        distance = getDistance()
        if distance is not None:
            total_error += abs(0.9 - distance)
        time.sleep(dt)
    sim.stopSimulation()
    time.sleep(1)  # 確保模擬完全停止
...
def is_balanced():
    distance = getDistance()
    return distance is not None and 0.8999 <= distance <= 0.9001
