...
PID控制器是目前最廣泛使用的控制器，利用回授系統將訊號回傳就可以利用與設定點的偏差得到\(e(t)\)值，接下來我們將介紹PID中P、I、D是如何對系統產生影響。\\
\begin{equation}
u(t) = K_p e(t) + K_I \int_0^t e(\tau) \, d\tau + K_D \frac{d e(t)}{d t}
\label{(6.12)}
\end{equation}\\
\subsubsection{比例控制律(Kp)}
在系統中加入\(K_p\)可以增快系統反應速度，但是有可能會造成系統響應阻尼比下降導致系統不穩定。\\
\begin{equation}
U_P(t)=K_P e(t)
\label{(6.13)}
\end{equation}\\
\subsubsection{積分控制律(Ki)}
\(K_i\)可以累計誤差，在系統反應初期作用不大，但經過一段時間的累積過後可以藉由先前累積的數據和設定值的平均誤差來讓系統穩定。\\
\begin{equation}
u(t)= K_I \int_0^t e(\tau) d \tau
\label{(6.14)}
\end{equation}\\
\subsubsection{微分控制律(Kd)}
